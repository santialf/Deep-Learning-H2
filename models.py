...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # compute the score
        score = torch.bmm(z, encoder_outputs.transpose(1,2))
        
        # our score matrix has 3 dimensions now, because we didn't implement the for loop
        # in the decoder, so now we have to add a new dimension to the mask, and make that
        # new dimension the same size as the dimension of 'z'
        src_seq_mask = src_seq_mask.unsqueeze(1)
        src_seq_mask = src_seq_mask.repeat(1, query.shape[1], 1)
        score = score.masked_fill(src_seq_mask, float("-inf"))

        # do the softmax of the score
        p = torch.softmax(score, 2)
        # compute the context vector
        c = torch.bmm(p, encoder_outputs)

        # compute the attention layer
        attn_out = self.linear_out(torch.cat((query, c), 2))
...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        packed_input = torch.nn.utils.rnn.pack_padded_sequence(emb, lengths, batch_first=True, enforce_sorted=False)

        output, final_hidden = self.lstm(packed_input)
        
        enc_output, ignore = torch.nn.utils.rnn.pad_packed_sequence(output, batch_first=True)
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        shape, ignore = tgt.shape

        # remove end of string
        tgt = tgt[EOS_mask]
        tgt = tgt.view(shape, -1)
